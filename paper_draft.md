...
<!-- BEGIN GENERATED paper_status -- do not edit by hand -->

> ## Status at regeneration
>
> *Generated by `scripts/paper_status.py` from the run JSONs. The prose below this block is hand-written and parts of it predate these measurements; where a section's figures have been superseded they are kept in place rather than edited away, so the reader can see what moved. `scripts/paper_status.py --check` fails when this block is older than the runs it reads.*
>
> **Clause 1 (rel-L2 <= 0.05).** Best in the `specprop` family now measured: **`m64_ma64` at 0.07282** terminal-step rel-L2 over 3 seed(s) (range 0.00140, 2,130,944 parameters) — **NOT MET**, short by 1.46x. 11 configurations scored and none meets the clause.
>
> **Clause 2 (>= 1000x).** Four readings, 8 repeats, both sides' CPU clocks measured in the same invocation:
>
> | reading | median | range | meets 1000x | verdict |
> |---|---|---|---|---|
> | raw, burst (the published protocol) | **1255.4x** | 568–1342 | 7/8 | STRADDLES the threshold |
> | raw, sustained (duration-matched) | **1225.5x** | 738–1366 | 6/8 | STRADDLES the threshold |
> | cycle-matched, burst | **1265.3x** | 811–1338 | 7/8 | STRADDLES the threshold |
> | cycle-matched, sustained | **1235.3x** | 948–1366 | 6/8 | STRADDLES the threshold |
>
> The two sides are not measured at the same clock: solver 3.93 GHz (3.00–4.00), operator 3.79 GHz (2.10–4.00). A `powersave` governor treats a ~0.5 s solve and a ~0.35 ms burst differently, so the ratio carries a clock term belonging to neither implementation.
>
> The same architecture, priced under different box conditions, meets the clause at very different rates — the architecture is not what differs between these:
>
> | run | loadavg | meets 1000x |
> |---|---|---|
> | `cost_repro_specprop_m4_ma64_K10.json` | 413–422 | 3/14 |
> | `cost_repro_specprop_m4_ma64_K10_IDLEBOX.json` | 18–25 | 1/14 |
>
> **Where the cost is.** `04_coeffs_a` is **243.1 us, 46.3%** of the forward — one `nn.Linear` holding 98.0% of the parameters. Every FFT together is 15.4%. Fifteen turns of cost work in this repo targeted the transforms.
>
> **The margin route, priced and bounded.** Factorising that layer buys **1.59x** on the whole forward at rank 4, and the route is capped at **1.68x** even at rank 1, because a measured floor in the stage does not shrink with the rank. **No accuracy is measured for any rank yet.**
>
> **Clause 3 (shape error <= 5%).** **0.0070** mean, p90 0.0107, max 0.0121, 100% of targets under 5% — **MET**, verified in the simulator.
>
> **Superseded in the prose below.** §4.2's headline table prices the FNO family against the fixed-duration denominator this repo retired in §4.2.1, and reports 9.58x. That number is a real measurement of a different architecture against a different denominator; it is not this repo's current clause-2 estimate and the abstract's "unreachable at 1000x under every reading we can defend" no longer describes the table above.

<!-- END GENERATED paper_status -->
